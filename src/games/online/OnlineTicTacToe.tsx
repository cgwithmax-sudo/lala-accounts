...
export default function OnlineTicTacToe({ currentUser }: Props) {
  ...
  async function createRoom() {
    ...
    try {
      ...
      setRoom(json.room); setRoomId(json.room.id); setPoll(true);
    } catch (e: any) { setErr(e.message); }
    finally { setCreating(false); }
  }

  async function joinRoom() {
    if (!roomId.trim()) { setErr('Enter a room ID'); return; }
    setErr(null); setJoining(true);
    try {
      const res = await fetch(`${API}/api/tictactoe/join`, {
        method: 'POST',
        headers: { 'Content-Type':'application/json' },
        body: JSON.stringify({ roomId: roomId.trim(), ...currentUser }),
      });
      const json = await res.json();
      if (!json.ok) throw new Error(json.error || 'Failed to join');
      setRoom(json.room); setPoll(true);
    } catch (e:any) { setErr(e.message); }
